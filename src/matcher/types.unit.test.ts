...
describe('Type Checking:', () => {
  for (const x of [
	{ name: 'Object',  func: types.object  },
	{ name: 'Number',  func: types.number  },
	{ name: 'Integer', func: types.integer },
	{ name: 'String',  func: types.string  },
	{ name: 'Boolean', func: types.boolean },
	{ name: 'Array',   func: types.array   }
  ]) {
	describe(`${x.name} Checking`, () => {
  	it('rejects null', () => {
		x.func(null).should.be.false;
		});
		 it('rejects undefined', () => {
		x.func(undefined).should.be.false;
		});
	});
  }
  ...
  describe('Presence Checking', () => {
	...
	it('rejects objects without the named property', () => {
		...
		/* on the object */
		types.anything(x["hopefully this isn't a property"])
		.should.be.false;
	});
  });
